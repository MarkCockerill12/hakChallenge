...
function setBudget() {
 ...
 function updateBudgetDisplay() {
   ...
   // Calculate total spent as the difference between original budget and current budget
  totalSpent = ogBudget - budget;
...
const jsonData = [
    ...
    function sortItemsByPriceLowToHigh() {
      const buyingSection = document.getElementById('buying');
      buyingSection.innerHTML = ''; // Clear previous items
    ...
    function displayItems(items) {
      ...
      items.forEach(item => {
          ...
          const pPrice = document.createElement('p');
          pPrice.textContent = `Price: $${item.price.toFixed(2)}`;
